...
- [x] **Mecanum cmd_vel bridge** — `slam_bringup.yahboom_bridge_node` subscribes `/cmd_vel`, calls `Rosmaster_Lib.set_car_motion(vx, vy, wz)` over `/dev/myserial`. Path A (direct Python bridge) per the Obsidian AutomaticAddison note — STM32 firmware does mecanum kinematics internally with `car_type=0x01`. Wired into `slam.launch.py` via `enable_drive:=true`. Standalone via `./start_yahboom.sh`. Bridge inverts `vx`/`vy` (params `invert_vx` / `invert_vy`, defaults true) to compensate for this rover's chassis being mounted with firmware-+X opposite the sensor mast — keeps Nav2's costmap / footprint / `/scan` aligned with the URDF rather than rotating `body→base_link`.
- [x] **Nav2 holonomic config** — `config/nav2_params.yaml` enables Y-axis sampling for DWB (`max_vel_y`, `vy_samples`, `acc_lim_y`, `decel_lim_y`, plus matching Y entries in `velocity_smoother`) so Nav2 plans strafe trajectories on the mecanum. Diff-drive platforms must override these to 0.0 in a per-platform params file.
